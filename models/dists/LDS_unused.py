...
class LinearDynamicalSystems():
    ...
    def reshape_inputs(self,y,u,r):
        # here sample_shape includes time and may include batch
        # if it does not include batch, then expand_y_batch should be True
        ...
        if(self.expand_to_batch==True):
            ...
            y=y.expand(sample_shape+self.batch_shape+self.obs_shape)
            if u is not None:
                u=u.expand(sample_shape+self.batch_shape+(self.control_dim-1,))
            if r is not None:
                r=r.expand(sample_shape+self.batch_shape+self.obs_shape[-2:-1]+(self.regression_dim-1,))
        else:
            sample_shape = y.shape[:-len(self.obs_shape)-len(self.batch_shape)]

        if u is None:
            u = torch.tensor(1.0,requires_grad=False).expand(sample_shape+self.batch_shape+self.offset+(1,1))
        else:
            u = torch.cat((u,torch.ones(u.shape[:-1]+(1,),requires_grad=False)),-1).unsqueeze(-1)
            for i in range(len(self.offset)):
                u=u.unsqueeze(-3)
            u = u.expand(sample_shape+self.batch_shape+self.offset+(self.control_dim,1))
        if r is None:
            r = torch.tensor(1.0,requires_grad=False).expand(sample_shape+self.batch_shape+self.offset+(1,1))
        else:
...
#                self.T=self.T.unsqueeze(-1)
    ...
    def update_latents(self,y,u,r,p=None,lr=1.0):
        ...
        invSigma, invSigmamu, Sigma, mu, fbw_Sigma_t_tp1, Sigma_x0_x0, mu_x0, logZ = self.forward_backward_loop(y,u,r)  # updates and stores self.px
        ...
        SE_xp_x = ((mu[:-1] @ mu[1:].transpose(-2,-1))).sum(0)  + (fbw_Sigma_t_tp1[:-1]).sum(0) #Sigma_0m1_0 now in last entry of fbw_Sigma_t_tp1
        SE_xp_x = SE_xp_x + mu_x0 @ mu[0].transpose(-2,-1) + fbw_Sigma_t_tp1[-1]

        SE_x_r =  ((mu@r.transpose(-2,-1))).sum(0)
        SE_x_y = ((mu@y.transpose(-2,-1))).sum(0)

        SE_u_u = ((u[1:]@u[1:].transpose(-2,-1))).sum(0) + (u[0]@u[0].transpose(-2,-1)) 
    ...
    def KLqprior(self):  # returns batch_size
        ...
        for i in range(len(self.offset)):
            KL = KL.squeeze(-1)
        return KL.sum(-1) + self.obs_model.KLqprior().sum(-1)
    ...
    def forward_step(self,invSigma, invSigmamu, Residual, invSigma_like, invSigmamu_like, Residual_like, U): 
        ...
        mu = torch.linalg.solve(invSigma,invSigmamu)
        ...
        return invSigma, invSigmamu, post_Residual, Residual
    ...
    def backward_step_with_Residual(self, invGamma, invGammamu, Residual, invSigma_like, invSigmamu_like, Residual_like, U):
        ...
        mu = torch.linalg.solve(invGamma,invGammamu)
...
###  Save on memory here by using px instead of allocating new invSigma....
#        if self.px is None:
#            self.px = MultivariateNormal_vector_format(invSigmamu = torch.zeros(sample_shape + self.batch_shape + self.offset + (self.hidden_dim,1),requires_grad=False),
...
#            )
        ...
        for t in range(T_max):
            invSigma[t], invSigmamu[t], Residual, logZ[t] = self.forward_step(invSigma[t-1], invSigmamu[t-1], Residual, invSigma_like[t], invSigmamu_like[t], Residual_like[t], u[t])
...
#            invGamma, invGammamu, Residual, logZ_b[t] = self.backward_step_with_Residual(invGamma, invGammamu, Residual, invSigma_like[t+1], invSigmamu_like[t+1],Residual_like[t+1],u[t+1])
            Sigma_t_tp1[t] = (invSigma[t] + self.ATQA_x_x).inverse() @ self.QA_xp_x.transpose(-2,-1) @ Sigma[t+1] #uses invSigma from tp1
...
#        invGamma, invGammamu, Residual, logZ_b[-1] = self.backward_step_with_Residual(invGamma, invGammamu, Residual, invSigma_like[t+1], invSigmamu_like[t+1],Residual_like[t+1],u[t+1])

        Sigma_t_tp1[-1] = (self.x0.EinvSigma() + self.ATQA_x_x).inverse() @ self.QA_xp_x.transpose(-2,-1) @ Sigma[0] #uses invSigma from tp1        
        ...
        return invSigma, invSigmamu, Sigma, mu, Sigma_t_tp1, Sigma_x0_x0, mu_x0, logZ
