...
Using this factorization, posteriors are all conditionally conjugate and inference can be performed using coordinate ascent updates on natural parameters. A single learning rate with maximum value of 1 can also be used to implement stochastic vb ala Hoffman 2013.  I recommend using lr = 0.5 in general or lr = mini_batch_size/total_number_of_minibatches.
...
To fit the model just use the update method: model.update(y,u,r,lr=1,iters=1)
      ...
      r.shape = (T, batch_shape, number_of_microscopic_objects, regression_dim) or r=None
...
Upon completion: 
      ...
      model.particular_assignment() is (T,batch_shape, number_of_microscopic_objects)
                    and give the map estimate of the particular assignment to environment (0), and boundary + object (object number n)
