...
For those interested in the nitty gritty.  The logic of the algorithm is based upon message passing structured by factor graphs.  The various modules correspond to the different parts of the factorized posterior over the parameters, i.e.  q(A,B) is a linear dynamics system posterior, q(C_lambda,D_lambda,T_lambda) is an ARHMM.  Associated with each model is also a estimate of the posterior over the latent variables associated with that module.  For the lds module that is q(x) while for the ARHMMs it is the q(lambda).  Modules upate their q(latents) using observations and q(latents_from_other_modules).  This particular model follows the Attend, Infer, Repeat paradigm (need reference) because it factorizes the posterior over 'assignment/attention' latents (lambdas) and hidden latents (x) and perfoms inference iterating between the two in the usual way.  
