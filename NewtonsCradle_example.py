...
model = DMBD(obs_shape=data.shape[-2:],role_dims=(8,4,8),hidden_dims=(4,2,4),batch_shape=(),regression_dim = 0, control_dim=0)
...
for i in range(iters):
    model.update(data,None,None,iters=2,latent_iters=1,lr=0.5+0.5*(i>iters/2),verbose=True)
...
batch_num = 40
plt.scatter(data[:,batch_num,:,0],data[:,batch_num,:,1],cmap='rainbow',c=model.obs_model.p.argmax(-1)[:,batch_num,:])
